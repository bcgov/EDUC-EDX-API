...
echo
echo Writing scope WRITE_EDX_USER_NAME
curl -sX POST "https://$SOAM_KC/auth/admin/realms/$SOAM_KC_REALM_ID/client-scopes" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TKN" \
  -d "{\"description\": \"Updating first name and last name in EDX_USER\",\"id\": \"WRITE_EDX_USER_NAME\",\"name\": \"WRITE_EDX_USER_NAME\",\"protocol\": \"openid-connect\",\"attributes\" : {\"include.in.token.scope\" : \"true\",\"display.on.consent.screen\" : \"false\"}}"
...
if [ "$envValue" = "prod" ]
then
  ROLES_ALLOW_LIST="EDX_DISTRICT_ADMIN,EDX_SCHOOL_ADMIN,SECURE_EXCHANGE_SCHOOL,SECURE_EXCHANGE_DISTRICT,EDX_EDIT_SCHOOL,EDX_EDIT_DISTRICT,DIS_SDC_RO,SCH_SDC_RO,SCHOOL_SDC,DISTRICT_SDC,SUPERINT,SECR_TRES,GRAD_SCH_ADMIN,GRAD_DIS_ADMIN,GRAD_SCH_RO,GRAD_DIS_RO"
