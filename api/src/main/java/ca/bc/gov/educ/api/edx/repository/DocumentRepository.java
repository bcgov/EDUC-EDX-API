...
public interface DocumentRepository extends JpaRepository<SecureExchangeDocumentEntity, UUID> {
  ...
  @Transactional
  @Modifying
  @Query(value = "delete from SECURE_EXCHANGE_DOCUMENT e where exists(select 1 from SECURE_EXCHANGE s where s.SECURE_EXCHANGE_ID = e.SECURE_EXCHANGE_ID and s.CREATE_DATE <= :createDate)", nativeQuery = true)
